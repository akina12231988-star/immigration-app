...
describe("sswGaps（通算に数えていない期間）", () => {
  it("通算対象の期間と期間のあいだの空白を返す", () => {
    const gaps = sswGaps(
      [
        h("特定活動（特定技能1号移行準備）", "2023-05-19", "2023-11-24"),
        h("特定技能1号", "2025-12-15", null),
      ],
      "2026-08-28",
    );
    expect(gaps).toEqual([{ start: "2023-11-25", end: "2025-12-14", days: 751 }]);
  });

  it("翌日から始まる期間・重なる期間は空白にしない", () => {
    expect(
      sswGaps(
        [
          h("特定技能1号", "2024-01-01", "2024-06-30"),
          h("特定技能1号", "2024-07-01", "2024-12-31"),
        ],
        "2026-08-28",
      ),
    ).toEqual([]);
    expect(
      sswGaps(
        [
          h("特定技能1号", "2024-01-01", "2024-06-30"),
          h("特定技能1号", "2024-05-01", "2024-12-31"),
        ],
        "2026-08-28",
      ),
    ).toEqual([]);
  ...
  it("前の期間に含まれてしまう期間があっても、そのあとに空白を作らない", () => {
    // 2024-01-01〜2024-12-31 の中に 2024-03-01〜2024-03-31 が入っている
    expect(
      sswGaps(
        [
          h("特定技能1号", "2024-01-01", "2024-12-31"),
          h("特定技能1号", "2024-03-01", "2024-03-31"),
          h("特定技能1号", "2025-01-01", null),
        ],
        "2026-08-28",
      ),
    ).toEqual([]);
  ...
  it("通算対象でない期間は空白を埋めない", () => {
    const gaps = sswGaps(
      [
        h("特定技能1号", "2024-01-01", "2024-06-30"),
        h("技能実習", "2024-07-01", "2024-12-31"),
        h("特定技能1号", "2025-01-01", null),
      ],
      "2026-08-28",
    );
    expect(gaps).toEqual([{ start: "2024-07-01", end: "2024-12-31", days: 184 }]);
  });

  it("在留資格を保持したままの帰国は空白にしない", () => {
    expect(
      sswGaps(
        [
          h("特定技能1号", "2024-01-01", "2024-06-30"),
          h("本国での職歴", "2024-07-01", "2024-12-31", true),
          h("特定技能1号", "2025-01-01", null),
        ],
        "2026-08-28",
      ),
  ...
  it("基準日より後に始まる期間は見ない・空白が無ければ空配列", () => {
    expect(
      sswGaps(
        [
          h("特定技能1号", "2024-01-01", null),
          h("特定技能1号", "2027-01-01", null),
        ],
        "2026-08-28",
      ),
    ).toEqual([]);
