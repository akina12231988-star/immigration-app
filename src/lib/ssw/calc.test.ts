...
describe("calcDocumentTotal（申請書類用の通算・1日でも在留した月は1か月）", () => {
  it("月の途中の開始・終了もその月を1か月と数える", () => {
    // 2022-09-01〜2023-06-01 → 2022年9月〜2023年6月の10か月
    expect(calcDocumentTotal([h("特定技能1号", "2022-09-01", "2023-06-01")], "2026-08-05")).toBe(10);
    // 月末〜翌月頭の2日間でも2か月
    expect(calcDocumentTotal([h("特定技能1号", "2024-01-31", "2024-02-01")], "2026-08-05")).toBe(2);
  });
  it("帰国などの空白期間は数えない（外国人詳細ページの実例）", () => {
    // 10か月 + 10か月 + 12か月 = 32か月（2年8か月）。帰国約1年5か月・転職空白40日は含まない
    const t = calcDocumentTotal(
      [
        h("技能実習", "2019-08-17", "2022-08-16"),
        h("特定技能1号", "2022-09-01", "2023-06-01"), // 2022/9〜2023/6 = 10か月
        h("本国での職歴", "2023-07-01", "2024-10-01"), // 対象外
        h("特定技能1号", "2024-11-02", "2025-08-01"), // 2024/11〜2025/8 = 10か月
        h("特定技能1号", "2025-09-11", null), // 2025/9〜2026/8 = 12か月
      ],
      "2026-08-05",
    );
    expect(t).toBe(32);
  });
  it("同じ月に複数の期間がかかっても1か月として数える", () => {
    const t = calcDocumentTotal(
      [
        h("特定技能1号", "2024-01-01", "2024-01-10"),
        h("特定技能1号", "2024-01-20", "2024-02-05"),
      ],
      "2026-08-05",
    );
    expect(t).toBe(2); // 2024年1月・2月
  });
  it("在留資格を保持したままの帰国は数え、切っての帰国は数えない", () => {
    expect(
      calcDocumentTotal([h("本国での職歴", "2024-01-01", "2024-01-31", true)], "2026-08-05"),
    ).toBe(1);
    expect(
      calcDocumentTotal([h("本国での職歴", "2024-01-01", "2024-01-31", false)], "2026-08-05"),
    ).toBeNull();
  });
  it("継続中は書類作成日の月まで・作成日より後に始まる期間は数えない", () => {
    const t = calcDocumentTotal(
      [
        h("特定技能1号", "2026-01-15", null), // 2026/1〜2026/3 = 3か月
