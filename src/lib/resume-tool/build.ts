...
const RESUME_CSS = `*{box-sizing:border-box;margin:0;padding:0}html,body{background:#e9eef4}body{font-family:'Noto Sans JP',sans-serif;color:#0f172a;-webkit-font-smoothing:antialiased;padding:14px 10px 40px}.bar{max-width:210mm;margin:0 auto 12px;display:flex;flex-wrap:wrap;align-items:center;gap:10px}.bar button{background:#1e3a5f;color:#fff;border:none;border-radius:10px;padding:12px 18px;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit;box-shadow:0 4px 12px rgba(30,58,95,.25)}.bar button:active{background:#2d5580}.bar span{font-size:11px;color:#64748b}.viewport{width:100%;overflow:hidden}.page{width:210mm;min-height:297mm;background:#fff;margin:0 auto;padding:14mm;border-radius:14px;box-shadow:0 12px 40px rgba(15,23,42,.14)}.rhead{display:flex;justify-content:space-between;align-items:flex-start;gap:18px;border-bottom:3px solid #1e3a5f;padding-bottom:12px;margin-bottom:14px}.rtitle h1{font-size:21px;font-weight:700;letter-spacing:.24em;color:#1e3a5f;line-height:1.3}.rtitle p{font-size:9px;letter-spacing:.16em;color:#94a3b8;margin-top:6px}.rphoto img,.rphoto>div{border-radius:6px}table{width:100%;border-collapse:collapse}table+table{margin-top:-1px}td,th{border:1px solid #cbd5e1;padding:7px 8px;vertical-align:middle;font-size:11.5px;line-height:1.55}.lc{background:#eef2f7;font-weight:700;font-size:10.5px;white-space:nowrap;text-align:center;color:#334155}.nb{display:inline-flex;align-items:center;justify-content:center;width:15px;height:15px;background:#1e3a5f;color:#fff;border-radius:50%;font-size:8px;margin-right:3px;font-weight:700}.rireki-data{color:#fff;font-size:4px;line-height:1.1;word-break:break-all;user-select:all;margin-top:6px}.howto{position:fixed;inset:0;background:rgba(15,23,42,.55);display:none;align-items:center;justify-content:center;padding:18px;z-index:50}.howto-box{background:#fff;border-radius:16px;max-width:420px;width:100%;padding:22px;max-height:88vh;overflow:auto}.howto-box h3{color:#1e3a5f;font-size:17px;margin-bottom:8px}.howto-box p{font-size:12.5px;color:#475569;margin-bottom:14px;line-height:1.6}.howto-box .primary{width:100%;background:#1e3a5f;color:#fff;border:none;border-radius:10px;padding:13px 14px;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit;margin-bottom:14px;line-height:1.4}.howto-steps{background:#f1f5f9;border-radius:10px;padding:14px;font-size:12.5px;color:#334155;line-height:1.7}.howto-steps ol{margin:6px 0 12px 18px}.howto-steps b{color:#1e3a5f}.howto-close{width:100%;background:none;border:none;color:#94a3b8;font-size:13px;margin-top:12px;cursor:pointer;font-family:inherit}@media print{html,body{background:#fff}body{padding:0}.bar,.howto{display:none!important}.viewport{overflow:visible;height:auto!important}.page{width:auto;min-height:0;margin:0;padding:0;border-radius:0;box-shadow:none;transform:none!important}tr,td,th{break-inside:avoid;page-break-inside:avoid}thead{display:table-header-group}table{break-inside:auto}@page{size:A4 portrait;margin:12mm}}`;

// スマホでは A4 を画面幅に縮めて全体を見せる
const FIT_SCRIPT = `function fit(){var p=document.querySelector('.page'),w=document.querySelector('.viewport');if(!p||!w)return;p.style.transform='';p.style.transformOrigin='top left';w.style.height='';var avail=w.clientWidth,pw=p.offsetWidth;if(avail<pw-1){var s=avail/pw;p.style.transform='scale('+s+')';w.style.height=(p.offsetHeight*s)+'px';}}window.addEventListener('load',fit);window.addEventListener('resize',fit);`;
...
export function buildResumeHtml(d: ResumeData, lang: ResumeLang, now: Date = new Date()): string {
  ...
  const emptyRow = `<tr><td class="lc"></td><td colspan="6" style="height:26px;"></td></tr>`;
  const cRows =
    careers
      .map(
        (c) =>
  ...
  return `<!DOCTYPE html><html lang="ja"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${e(d.name || "履歴書")} 履歴書</title><link href="https://fonts.googleapis.com/css2?family=Noto+Sans+JP:wght@400;700&display=swap" rel="stylesheet"><style>${RESUME_CSS}</style></head><body><div class="bar"><button onclick="document.getElementById('howto').style.display='flex'">📄 PDFに保存 / 印刷</button><span>スマホは全体表示・PDFはA4で保存されます</span></div>${HOWTO_HTML}<div class="viewport"><div class="page"><header class="rhead"><div class="rtitle"><h1>特定技能外国人の履歴書</h1><p>SPECIFIED SKILLED WORKER · RESUME</p></div><div class="rphoto">${photoHtml(d.photo)}</div></header><table><tr><td class="lc" rowspan="2" style="width:52px;"><span class="nb">①</span>氏名</td><td class="lc" style="width:56px;">フリガナ</td><td colspan="2">${e(d.kana)}</td><td class="lc" style="width:64px;"><span class="nb">②</span>性別</td><td colspan="2">${e(d.gender)}</td></tr><tr><td class="lc">英字</td><td colspan="2">${e(d.name)}</td><td class="lc"><span class="nb">③</span>生年月日</td><td colspan="2">${e(formatResumeDate(d.dob))}${ageHtml(d.dob, now)}</td></tr><tr><td class="lc" colspan="2"><span class="nb">④</span>国籍・地域</td><td>${e(d.nat)}</td><td class="lc"><span class="nb">⑤</span>十分に理解できる言語</td><td>${e(d.lang)}</td><td class="lc"><span class="nb">⑥</span>配偶者の有無</td><td style="text-align:center;">${e(d.spouse)}</td></tr><tr><td class="lc" colspan="2"><span class="nb">⑦</span>実習の職種</td><td colspan="2">${e(d.jtype)}</td><td class="lc"><span class="nb">⑧</span>実習の作業名</td><td colspan="2">${e(d.jwork)}</td></tr><tr><td class="lc" colspan="2">実習修了日</td><td colspan="2">${e(formatResumeDate(d.tend))}</td><td class="lc">ビザの期限</td><td colspan="2">${e(formatResumeDate(d.vexp))}</td></tr><tr><td class="lc" colspan="2">現在の在留資格</td><td colspan="5">${e(d.status)}</td></tr><tr><td class="lc" colspan="2">日本での現在住居地</td><td colspan="5">${e(d.adjp)}</td></tr><tr><td class="lc" colspan="2">本国の住居地</td><td colspan="5">${e(d.adhm)}</td></tr></table><table><thead><tr><th class="lc" style="width:52px;"><span class="nb">⑨</span>職歴</th><th class="lc">開始</th><th class="lc">終了</th><th class="lc" colspan="2">会社名</th><th class="lc">特定技能分野</th><th class="lc">当時の在留資格</th></tr></thead><tbody>${cRows}<tr><td class="lc"></td><td colspan="6" style="height:16px;"></td></tr></tbody></table><table><tr><td class="lc" colspan="2"><span class="nb">⑩</span>資格・免許</td><td colspan="5" style="white-space:pre-wrap;min-height:40px;">${e(d.lic || "　－ 専門級、技能実習終了証明書")}</td></tr><tr><td class="lc" rowspan="5">体の状態</td><td class="lc">身長</td><td>${e(d.ht)} cm</td><td class="lc" rowspan="5">その他</td><td class="lc">利き手</td><td colspan="2">${e(d.hand)}</td></tr><tr><td class="lc">体重</td><td>${e(d.wt)} kg</td><td class="lc">趣味</td><td colspan="2">${e(d.hob)}</td></tr><tr><td class="lc">血液型</td><td>${e(d.bl)}</td><td class="lc">飲酒</td><td colspan="2">${e(d.drink)}</td></tr><tr><td class="lc">病気</td><td>${e(d.ill)}</td><td class="lc">タバコ</td><td colspan="2">${e(d.smoke)}</td></tr><tr><td class="lc">視力</td><td>右:${e((vis[0] ?? "").trim())} 左:${e((vis[1] ?? "").trim())}</td><td colspan="3"></td></tr></table><table><thead><tr><th class="lc" style="width:52px;">家族構成</th><th class="lc">続柄</th><th class="lc" colspan="2">氏名</th><th class="lc">年齢</th><th class="lc" colspan="2">仕事</th></tr></thead><tbody>${fRows}<tr><td class="lc"></td><td colspan="6" style="height:16px;"></td></tr></tbody></table><div class="rireki-data">${RIREKI_EMBED_START}${payloadB64}${RIREKI_EMBED_END}</div></div></div><script>${FIT_SCRIPT}</script></body></html>`;
}
