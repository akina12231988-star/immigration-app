...
describe("対象月の時点で有効だった在留許可", () => {
  ...
  it("許可時の在留資格に入力の揺れがあっても、更新なら名簿の在留資格は書き換えない", () => {
    // 「特定技能1号更新」のような入力がそのまま名簿に出ないようにする
    const permits = [permit({ permitDate: "2026-08-05", visa: "特定技能1号更新" })];
    expect(effectivePermitForMonth(permits, "2026-07")?.visa).toBe("");
  });
