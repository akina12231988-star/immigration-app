...
export function WorkerContracts({
  ...
  const handleError = (err: unknown) => {
    ...
    // 0079未適用だと worker_documents の kind 制約で弾かれる
    if (/check|制約/i.test(message)) {
      message += "／マイグレーション 0079_recruit_ledgers.sql が未適用の可能性があります。Supabase の SQL Editor で適用してください。";
    }
    setError(message);
  };
    ...
    <section id="contracts">
      <Card className="p-4">
        ...
        <p className="mb-3 text-[11px] leading-relaxed text-muted">
          採用時の雇用契約書・雇用条件書をこの外国人に紐づけて保管します。PDF・画像で登録でき、
          ダウンロードして Messenger で本人に送れます。新しく登録すると最新になり、以前の分も履歴に残ります。
        </p>
        {error && (
