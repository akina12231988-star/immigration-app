...
// 住所歴の根拠となる画像・PDF（住民票やマイナポータルのスクショなど）の添付。
// 非公開バケット app-files（address-files/）に保存し、署名付きURLで
...
// アップロード用の署名付きURLを発行
export async function createAddressFileTicket(
  workerId: string,
  fileName: string,
  mimeType: string,
  ...
  const path = `address-files/${workerId}/${crypto.randomUUID()}.${ext}`;
...
export async function registerAddressFile(
  workerId: string,
  addressId: string,
  path: string,
  fileName: string,
  ...
  if (!(await requireStaff())) return { ok: false, message: "権限がありません" };
  if (!path.startsWith(`address-files/${workerId}/`)) {
    return { ok: false, message: "不正なパス" };
  }
  const me = await getMyProfile();
  const admin = createAdminClient();
  if (!admin) return { ok: false, message: "サーバー設定エラー" };
  const { error } = await admin.from("worker_address_files").insert({
    worker_id: workerId,
...
// 表示用の署名付きURL
export async function getAddressFilePreviewUrl(
  fileId: string,
...
export async function deleteAddressFile(fileId: string): Promise<{ ok: true } | Err> {
  if (!(await requireStaff())) return { ok: false, message: "権限がありません" };
  const admin = createAdminClient();
  if (!admin) return { ok: false, message: "サーバー設定エラー" };
  const { data } = await admin
  ...
  const { error } = await admin.from("worker_address_files").delete().eq("id", fileId);
  if (error) return { ok: false, message: error.message };
  return { ok: true };
}
