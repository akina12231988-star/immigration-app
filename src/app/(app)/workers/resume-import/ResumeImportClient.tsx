...
import { CheckCircle2, ExternalLink, FileSearch, Loader2, UserPlus, Users } from "lucide-react";
...
export function ResumeImportClient({ workers }: { workers: WorkerWithOrg[] }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 本人へ履歴書ツールを案内する（案内文・リンク・QR） */}
      <Card className="space-y-2 p-4">
        <p className="text-sm font-bold">履歴書ツール（本人がスマホで入力する画面）</p>
        <p className="text-xs leading-relaxed text-muted">
          本人にリンクを送ると、ログインなしで6言語（日本語・インドネシア語・ベトナム語・クメール語・タガログ語・英語）の画面から
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <a
            ...
            target="_blank"
            rel="noopener"
            className="inline-flex min-h-[52px] items-center gap-1.5 rounded-xl border border-border px-4 text-sm font-bold"
          >
