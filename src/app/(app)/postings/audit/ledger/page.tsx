...
import { postingLedgerTable, seekerLedgerTable } from "@/lib/recruit-ledgers";
...
export default async function AuditLedgerPrintPage({
  ...
  const [postings, seekers] = await Promise.all([
  ...
  const seekerEntries = filterSeekerLedger(seekers, auditPairs(postings, targets));
