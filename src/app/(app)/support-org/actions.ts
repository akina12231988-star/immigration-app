...
// 登録支援機関（当社）の添付ファイル（0153_support_org_files.sql）。
// 人材サービス総合サイトの掲載画像など。非公開バケット app-files（support-org-files/）に保存し、
// 署名付きURLでアップロード・表示する（所属機関の添付と同じ方式）。
...
export interface SupportOrgFileView {
  id: string;
  kind: string;
  file_name: string;
  mime_type: string;
...
// その種類の添付を新しい順に取得
export async function listSupportOrgFiles(kind: string): Promise<SupportOrgFileView[]> {
  const me = await getMyProfile();
  const admin = createAdminClient();
  if (!me || !admin) return [];
  const { data, error } = await admin
    .from("support_org_files")
    .select("id, kind, file_name, mime_type, created_at")
    .eq("kind", kind)
    .order("created_at", { ascending: false });
  if (error) throw new Error(error.message);
  return (data as SupportOrgFileView[]) ?? [];
}

// アップロード用の署名付きURLを発行
export async function createSupportOrgFileTicket(
  fileName: string,
  mimeType: string,
  ...
  const path = `support-org-files/${crypto.randomUUID()}.${ext}`;
...
export async function registerSupportOrgFile(
  kind: string,
  path: string,
  fileName: string,
  mimeType: string,
  ...
  if (!(await requireStaff())) return { ok: false, message: "権限がありません" };
  if (!path.startsWith("support-org-files/")) return { ok: false, message: "不正なパス" };
  const me = await getMyProfile();
  const admin = createAdminClient();
  if (!admin) return { ok: false, message: "サーバー設定エラー" };
  const { error } = await admin.from("support_org_files").insert({
    kind,
    storage_path: path,
    file_name: fileName,
    mime_type: mimeType,
...
// 表示用の署名付きURL
export async function getSupportOrgFilePreviewUrl(
  fileId: string,
...
export async function deleteSupportOrgFile(fileId: string): Promise<{ ok: true } | Err> {
  if (!(await requireStaff())) return { ok: false, message: "権限がありません" };
  const admin = createAdminClient();
  if (!admin) return { ok: false, message: "サーバー設定エラー" };
  const { data } = await admin
  ...
  const { error } = await admin.from("support_org_files").delete().eq("id", fileId);
  if (error) return { ok: false, message: error.message };
  return { ok: true };
}
