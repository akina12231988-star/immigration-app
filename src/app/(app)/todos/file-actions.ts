...
// TODOの添付ファイル（発行されたアプリケーションNo.のPDF・画像など）。
// 非公開バケット app-files（todo-files/）に保存し、署名付きURLで
// アップロード・表示する（求人の添付ファイル 0094 と同じ方式・0109）。
...
// アップロード用の署名付きURLを発行
export async function createTodoFileTicket(
  todoId: string,
  fileName: string,
  mimeType: string,
  ...
  const path = `todo-files/${todoId}/${crypto.randomUUID()}.${ext}`;
...
export async function registerTodoFile(
  todoId: string,
  kind: string,
  path: string,
  fileName: string,
  ...
  if (!(await requireStaff())) return { ok: false, message: "権限がありません" };
  if (!path.startsWith(`todo-files/${todoId}/`)) {
    return { ok: false, message: "不正なパス" };
  }
  const me = await getMyProfile();
  const admin = createAdminClient();
  if (!admin) return { ok: false, message: "サーバー設定エラー" };
  const { error } = await admin.from("todo_files").insert({
    todo_id: todoId,
    kind,
    storage_path: path,
    file_name: fileName,
...
// 表示用の署名付きURL
export async function getTodoFilePreviewUrl(
  fileId: string,
...
export async function deleteTodoFile(fileId: string): Promise<{ ok: true } | Err> {
  if (!(await requireStaff())) return { ok: false, message: "権限がありません" };
  const admin = createAdminClient();
  if (!admin) return { ok: false, message: "サーバー設定エラー" };
  const { data } = await admin
  ...
  const { error } = await admin.from("todo_files").delete().eq("id", fileId);
  if (error) return { ok: false, message: error.message };
  return { ok: true };
}
