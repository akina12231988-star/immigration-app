...
//
// ツールは本人の入力（会社名・住所・趣味など、内蔵辞書で日本語にできない自由記述）を
...
function originAllowed(origin: string | null): boolean {
  return isTranslateOriginAllowed(origin, process.env.TRANSLATE_ALLOWED_ORIGINS);
}

function corsHeaders(origin: string | null): Record<string, string> {
  return {
    "Access-Control-Allow-Origin": originAllowed(origin) ? (origin as string) : DEFAULT_TRANSLATE_ORIGINS[0],
...
export async function POST(req: NextRequest) {
  ...
  if (!originAllowed(origin)) {
    return json({ error: "この呼び出し元からは使えません" }, 403, origin);
  }
  if (!process.env.ANTHROPIC_API_KEY) {
    return json({ error: "翻訳サーバーの設定がありません（ANTHROPIC_API_KEY）" }, 503, origin);
  }
  ...
  try {
    ...
    return json({ error: "不正なリクエストです" }, 400, origin);
  }
  ...
  if (!texts) return json({ error: "不正なリクエストです" }, 400, origin);

  const entries = pickTranslateTexts(texts);
  if (entries.length === 0) return json({ translations: {} }, 200, origin);
